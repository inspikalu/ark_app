...
import { FiSearch, FiHelpCircle, FiPlusCircle, FiMessageCircle, FiFilter, FiDollarSign, FiLock, FiCheckCircle } from "react-icons/fi";
...
const DashboardSearch: React.FC = () => {
  ...
  const handleManagementSelection = (type: 'chat' | 'dashboard') => {
    ...
    // We are using a setTimeout function to ensure the first modal is fully closed before opening the second
    setTimeout(() => {
  ...
  const actionButtons = [
    { icon: <FiPlusCircle />, text: "Create PAO", onClick: openManagementModal },
    { icon: <FiMessageCircle />, text: "Enter PAO Chat", onClick: handleEnterPAOChat },
    { icon: <FiFilter />, text: "Filter PAOs", onClick: () => {/* Implement filter functionality */} },
    { icon: <FiDollarSign />, text: "Squads Treasury", onClick: () => router.push('/multisig') },
    { icon: <FiCheckCircle />, text: "Reclaim Verify", onClick: () => router.push('/verify') },
    { icon: <FiLock />, text: "Escrow", onClick: () => router.push('/escrow') },
  ];
